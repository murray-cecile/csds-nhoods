...
def main(j, st):
    ...
    iter_csv = pd.read_csv(DATADIR + 'u_{:02d}.csv.bz2'.format(j), chunksize = 100000, 
                           names = ["publisher", "advertising_id", "timestamp", "latitude", "longitude", "accuracy", "opt"],
                           dtype = {"publisher" : str, "advertising_id" : str, "timestamp" : int, "latitude" : float, "longitude" : float, "accuracy" : int})
